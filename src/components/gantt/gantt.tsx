...
export const Gantt = forwardRef<GanttRef, GanttProps>(
  (
    ...
    const jumpToNow = () => {
      const now = new Date();
      // Find the column containing current time
      const index = dateSetup.dates.findIndex(
        (d, i) =>
      ...
      if (index !== -1) {
        ...
        const cellStartTime = dateSetup.dates[index].getTime();
        const cellEndTime =
          index + 1 !== dateSetup.dates.length
            ? dateSetup.dates[index + 1].getTime()
            : cellStartTime +
              (dateSetup.dates[index].getTime() -
                dateSetup.dates[index - 1 >= 0 ? index - 1 : 0].getTime());
        ...
        const exactX = index * columnWidth + boundedPosition * columnWidth;

        // Center the view on the current time
        const centerX = Math.max(0, exactX - svgContainerWidth / 2);
        setScrollX(centerX);
      }
    };
